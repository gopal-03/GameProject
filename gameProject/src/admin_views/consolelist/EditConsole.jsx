...
const EditConsole = () => {
  ...
  const fetchConsoleDetails = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8080/consoles/getdetail/${id}`
      );
      setConsoleName(response.data.consoleName);
      setNoOfDevices(response.data.noOfDevices);
      setPricePerHour(response.data.pricePerHour);
      setMembershipOffer(response.data.membershipOffer);
    } catch (error) {
  ...
  const updateConsole = async (e) => {
    ...
    try {
      await axios.put(
        `http://localhost:8080/consoles/update/${id}/${username}`,
        consoleDetails
      );
      navigate("/admin/dashboard/consolelist/");
    } catch (error) {
      ...
      <form onSubmit={updateConsole}>
        ...
        <div className="mb-3">
          <label className="form-label">
            No. of Available Devices in your Store
          </label>
          <input
            type="number"
